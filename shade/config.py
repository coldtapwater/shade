...
ABOUT_SHADE = "shade is a CLI to interact with large language models in a Chat-style interface, enabling the assistant to execute commands, interact with the user's system and code on the local machine, letting them assist in all kinds of development and terminal-based work."
...
config_path = os.path.expanduser("~/.config/shade/config.toml")
...
def get_workspace_prompt(workspace: str) -> str:
    project_config_paths = [
        p
        for p in (
            Path(workspace) / "shade.toml",
            Path(workspace) / ".github" / "shade.toml",
        )
        if p.exists()
    ]
