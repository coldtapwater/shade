...
def main(debug: bool, verbose: bool, model: str | None):  # pragma: no cover
    """
    Starts a server and web UI for shade.
    ...
    try:
        ...
        logger.error(
            "shade installed without needed extras for server. "
            "Install them with `pip install shade[server]`"
        )
        exit(1)
