...
import shade.cli
import shade.constants
import pytest
from click.testing import CliRunner
from shade.tools import ToolUse
...
def test_help(runner: CliRunner):
    result = runner.invoke(shade.cli.main, ["--help"])
    assert result.exit_code == 0


def test_version(runner: CliRunner):
    result = runner.invoke(shade.cli.main, ["--version"])
    assert result.exit_code == 0
...
def test_command_exit(args: list[str], runner: CliRunner):
    args.append("/exit")
    result = runner.invoke(shade.cli.main, args)
...
def test_command_summarize(args: list[str], runner: CliRunner):
    ...
    print(f"running: shade {' '.join(args)}")
    result = runner.invoke(shade.cli.main, args)
    assert result.exit_code == 0
...
def test_stdin(args: list[str], runner: CliRunner):
    args.append("/exit")
    print(f"running: shade {' '.join(args)}")
    result = runner.invoke(shade.cli.main, args, input="hello")
    assert "```stdin\nhello\n```" in result.output
    assert result.exit_code == 0
...
def test_tmux(args: list[str], runner: CliRunner):
    """
    $ shade '/impersonate lets find out the current load
    ```tmux
    new_session top
    ```'
    """
